...
parser$add_argument(
  "--config-file",
  type = "character",
  default = "/humgen/atgu1/methods/mkanai/workspace/202011_meta_finemapping/meta_config.tsv"
)
parser$add_argument(
  "--sumstats-format",
  type = "character",
  default = "/humgen/atgu1/methods/mkanai/workspace/202011_meta_finemapping/assoc/pheno%d/%s.PHENO.glm.linear.gz"
)
parser$add_argument(
  "--out-format",
  type = "character",
  default = "/humgen/atgu1/methods/mkanai/workspace/202011_meta_finemapping/meta_analysis/meta.pheno%d.config%d.tsv"
)
...
df <- purrr::map_dfr(studies, function(x) {
  ...
  data.table::fread(fname, data.table = F) %>%
    ...
    dplyr::rename(variant = "#ID", a1count = A1_CT, n = OBS_CT, beta = BETA, se = SE, p = P)
})
...
fname <- sprintf(args$out_format, args$pheno, args$config)
