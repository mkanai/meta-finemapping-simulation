...
def default_lift_data(
    ...
    t = (t.annotate(**liftover_expr(t.locus, t.alleles, target_genome)) if isinstance(t, hl.Table) else t.annotate_rows(
        **liftover_expr(t.locus, t.alleles, target_genome)))
    ...
    if flip_ref_alt:
        row_annot_expr = {'new_alleles': hl.if_else(t.ref_alt_flip, [t.new_alleles[1], t.new_alleles[0]], t.new_alleles)}
...
def main(args):
    ...
    if 'beta' in ht.row:
        ht = ht.annotate(beta=hl.if_else(ht.ref_alt_flip, -ht.beta, ht.beta))
    ht = ht.annotate(variant=hl.variant_str(ht.locus, ht.alleles),
                     original_variant=hl.variant_str(ht.original_locus, ht.original_alleles))
    ht = ht.key_by().drop('locus', 'alleles', 'new_locus', 'new_alleles', 'original_locus', 'original_alleles',
                          'locus_fail_liftover', 'ref_allele_mismatch')
